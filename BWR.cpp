...
int main() {
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      std::printf("%s  Densitatea (t,p)                       :%s %s%10.4f%s [kg/m\xC2\xB3]\n",
                  kBoldWhite, kReset,
      ...
      std::printf("%s  Debitul masic (t,p)                    :%s %s%10.4f%s [kg/s]\n",
                  kBoldWhite, kReset, kBoldGreen, qm, kReset);
      for (int i = 0; i < ref.n; i++) {
        double qhref = kSecondsPerHour / ror_ref[i] * qm;
        std::printf("%s  Debitul vol.  (%5.2f\xC2\xB0""C / 101.325 kPa)  :%s %s%8.2f%s [%s]\n",
                    kBoldWhite, ref.t[i], kReset, kBoldGreen, qhref, kReset, ref.label[i]);
      }
      std::printf("%s  Debitul vol.  (t,p)                    :%s %s%8.2f%s [m\xC2\xB3/h]\n",
                  kBoldWhite, kReset, kBoldGreen, kSecondsPerHour / ro * qm, kReset);
      std::printf("%s  Viteza medie a gazului                 :%s %s%8.2f%s [m/s]\n",
                  kBoldWhite, kReset, kBoldGreen, flow.viteza, kReset);
      std::printf("%s  Pierderea de presiune prin str\xC3\xA2ngulare :%s %s%8.2f%s [kPa]\n",
